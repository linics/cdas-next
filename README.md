...
```bash
# 官方语料索引与源 Markdown 一致
pnpm knowledge:check

# 静态检查、纯领域测试和生产构建（含 knowledge:check）
pnpm check
...
助手使用 AI SDK 官方 `useChat`、`streamText` 与签名 `toolApproval`，没有自建聊天协议或审批状态机。只读工具 `search_knowledge` 与 `read_source_section` 查询仓库内静态官方语料索引，不写业务表、不用向量库。草稿工具调用现有 `saveActivityDraft`；发布工具在 AI SDK 交互暂停之外，仍必须建立并消费绑定精确草稿版本、班级和截止时间的 ActionIntent。任何模型调用都不在数据库事务中，关闭或中断模型不会影响手工闭环。
...
- [corpus/official-standards/README.md](./corpus/official-standards/README.md)：首版官方课程标准语料边界与可复现构建
- `src/server/knowledge/`：只读词法检索与语料索引
