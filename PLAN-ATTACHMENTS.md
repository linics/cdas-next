...
每张 384 token 的上限值得注意：这是个**很小**的预算，意味着模型看到的是缩略理解而非精读。
用它做"这张图里有什么"够用，做"读出图里的手写公式第三步错在哪"不够。规划里要按前者定位。
...
标着 experimental。若不接受，第三期的图片部分要推迟到它转正，
PDF / docx 的文本抽取部分则不受影响、可以照做。
