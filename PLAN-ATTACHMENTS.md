...
状态：**调研与规划，未立项，不得开工。** 需要先做出第 4 节列出的三个决策。
...
4. **图片用 `<img>`，PDF 用 PDF.js。** 图片不需要任何库。PDF 第一期可以先用 `<iframe>`
   验证链路，但正式交付用自托管 PDF.js——VPS 上没有服务端渲染预算，客户端渲染是对的。
...
2. **抽出魔数校验。** `isExpectedSignature` 目前长在 Vercel Blob 的文件里，
   ...
   **本机存储没有 Blob 的病毒扫描，魔数校验是它唯一的内容防线**，绝不能有两份实现。

3. **`getScanDecision` 在本机的语义要显式决定。** Vercel Blob 那边它映射到平台扫描。
   本机没有扫描器。不要默默返回 `READY` 假装扫过了——要么接一个本地扫描器（ClamAV，
   但 2G 内存装不下），要么显式记录"本机部署不做病毒扫描，只做魔数校验"作为一条决策，
   并让 `SELF-HOST.md` 写明这个降级。**这是安全边界的实质变更，必须进 `DECISIONS.md`。**
...
1. **新增工具 `read_submission_attachment`。** 复用
   `getAuthorizedSubmissionAttachmentDownload`——**访问控制已经是对的，不要另写一套**。
   工具的边界与现有 11 个工具一致，按 `AGENT.md` 的工具边界规矩写。
...
3. **PDF 与 docx 走文本抽取，不走视觉。** 视觉 API 不收 PDF。
   - PDF → 服务端抽文本（`pdfjs-dist` 的 text layer 可在 Node 跑）。
   - docx → `mammoth` 抽纯文本（这正是它的强项）。
   - **扫描件（图片型 PDF）第一期明确不支持**，要给教师一句明确的界面说明，
     不要让它静默返回空文本——那会让 AI 基于"这份 PDF 是空的"起草评价。
...
教师确认链不变（AI 建议未经确认不保存）；真实模型冒烟连续通过。
...
**决策 1：本机存储做不做？** 不做则附件永远只在 Vercel 可用，VPS 演示时这块是灰的。
做则是本轮最大工程量。考虑到演示在 VPS 上，倾向做——但这是产品优先级判断，不是工程判断。

**决策 2：本机部署不做病毒扫描，能否接受？** 2G 内存装不下 ClamAV。
只靠魔数校验意味着：一个真的是合法 PNG 文件头的恶意文件能过。
使用者是同一所学校的师生，不是开放互联网——风险可能可接受，但必须是**明写的决定**，
不能是默默的降级。

**决策 3：视觉模型用实验版是否可接受？** `deepseek-v4-flash-vision-exp` 上线 8 天，
标着 experimental。**这一条已被实测部分回答**：它在真实作业附件上读对了全部事实、
延迟正常、比 GLM 更省 token（见上表，`pnpm probe:vision` 可复跑）。剩下的风险不是能力，
而是"实验接口可能变更或下线"。若不接受这个风险，第三期的图片部分要推迟，
PDF / docx 的文本抽取部分不受影响、可以照做。
