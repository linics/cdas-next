...
状态：**切片 A 已按 D-044 交付；切片 B 的全局呈现层已按 D-045 交付，首批页面上下文与工作区只读摘要已按 D-046 交付，本人草稿正文只读按 D-047 交付，档 3 的首个写工具（草稿改写）按 D-048 交付；过程诊断按 D-051 交付；档 2 余下的发布/提交学生层明细尚未开放**。
...
> 2026-08-28 更新：D-045 已交付“教师端悬浮入口 + 非模态独立会话面板 + provider 上提”。D-046 接受首批请求级页面上下文与本人班级、草稿、发布/待办摘要；只返回 canonical href 并由教师点击。D-047 再放开本人某一份草稿的当前 v2 任务书正文，用于诊断与改写建议，仍是只读。D-048 进入档 3：教师确认后可把这些建议写成同一份草稿的新版本，沿用签名 approval 与既有 `saveActivityDraft`，并要求模型如实声明改动区域、由服务端与真实差异逐条核对。学生明细、发布提交名册、评价与反馈数据和过程诊断仍不进入全局 Agent。
