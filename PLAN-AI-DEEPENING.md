...
状态：**切片 A 已按 D-044 交付；切片 B 的全局呈现层已按 D-045 交付，首批页面上下文与工作区只读摘要已按 D-046 交付，本人草稿正文只读按 D-047 交付；档 2 余下的发布/提交明细与过程诊断尚未开放**。
...
> 2026-08-28 更新：D-045 已交付“教师端悬浮入口 + 非模态独立会话面板 + provider 上提”。D-046 接受首批请求级页面上下文与本人班级、草稿、发布/待办摘要；只返回 canonical href 并由教师点击。D-047 再放开本人某一份草稿的当前 v2 任务书正文，用于诊断与改写建议，仍是只读；学生明细、发布提交名册、评价与反馈数据和过程诊断仍不进入全局 Agent。
