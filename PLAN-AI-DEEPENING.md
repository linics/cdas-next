...
状态：**计划，未开工，未形成决策**。本文只描述打算怎么做与边界在哪，不是已接受的实现范围。
两个切片各自需要一条 `DECISIONS.md` 记录（暂记 D-044 / D-045）才能进入开发。
...
`codex/ui-redesign-kanban` 目前领先 `main` 20 个提交、74 个文件、+8975/−766，内容是视觉重做、
第 1.5 步情境叙事、3a 学生任务页重构、3b 教师端收束与演示种子重写。**两个 AI 切片都不在这条分支上开工**，
先把它验证掉：

- [ ] `pnpm check`（typecheck + lint + test）
- [ ] `python3 scripts/ui-audit/audit.py 1280 768 390` 输出为空
- [ ] 手动过一遍主路径：教师工作台 → 活动设计 → 发布确认 → 评阅名册 → 反馈与评价；学生端列表 → 任务页 → 提交 → 读反馈
- [ ] `AI_PROVIDER_DISABLED=1` 下上述手工路径全程可用
- [ ] 合入 `main`

验证期间发现的问题一律在本分支修，不要开新分支并行改 AI，否则演示分支和功能分支会互相追赶。
...
| 8/31 | 录备用视频 + 材料初稿（硬锚点） |
| 9/1 | 缓冲、彩排、终校（硬锚点） |
| 9/2 起 | 切片 A（补 D-044）→ 切片 B 档 1（补 D-045）→ 档 2 → 档 3 |
