...
export async function reconcileDevelopmentInfrastructure(config: DevelopmentInfrastructureConfig, providers: InfrastructureProviders, progress: InfrastructureProgress = () => undefined): Promise<InfrastructureResult> {
  ...
  const [teacher, student, otherStudent, otherTeacher] = await Promise.all([
    ...
    providers.clerk.ensureSyntheticIdentity(syntheticExternalIds.otherTeacher, syntheticUsernames.otherTeacher, "CDAS Staging Synthetic Other", "Teacher"),
  ]);
  if (new Set([teacher.id, student.id, otherStudent.id, otherTeacher.id]).size !== 4) throw new Error("DEVELOPMENT_INFRA_CLERK_IDENTITIES_NOT_DISTINCT");
  ...
  const secretsForGitHub: Record<string, string> = {
    ...
    STAGING_TEST_OTHER_TEACHER_CLERK_ID: otherTeacher.id,
    STAGING_HEALTH_PROOF_SECRET: secrets.healthProofSecret,
