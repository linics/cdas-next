...
function providers(events: string[], failAt?: string): InfrastructureProviders {
  ...
  return {
    ...
    vercel: { assertProject: async () => hit("vercel-project"), ensurePreviewEnvironment: async (values) => { hit("vercel-env"); expect(values.AI_PROVIDER_DISABLED).toBe("1"); expect(values).not.toHaveProperty("DEEPSEEK_API_KEY"); }, ensureProtectionBypass: async () => hit("vercel-bypass"), deployPreview: async (target) => { hit("vercel-deploy"); return { url: "https://cdas-next-test.vercel.app", sha: target.sha }; } },
