...
export const expectedEvidenceChecks = {
  "staging-preflight.v1": [
    "STAGING_RUN_MARKER", "STAGING_SYNTHETIC_DATA_ACK", "NODE_ENV_PRODUCTION", "STAGING_BASE_URL_HTTPS_REMOTE", "DATABASE_URL_REMOTE_POOLED", "DIRECT_URL_REMOTE_DIRECT", "DATABASE_URL_TLS_REQUIRED", "DIRECT_URL_TLS_REQUIRED", "STAGING_DATABASE_NAME_ACKNOWLEDGED", "DATABASE_RUNTIME_DIRECT_TARGETS_DISTINCT", "DATABASE_RUNTIME_DIRECT_CLUSTER_MATCH", "DATABASE_URL_ISOLATED_FROM_TEST_AND_E2E", "CLERK_TEST_PUBLISHABLE_KEY", "CLERK_TEST_SECRET_KEY", "STAGING_TEST_TEACHER_CLERK_ID", "STAGING_TEST_STUDENT_CLERK_ID", "STAGING_TEST_CLERK_IDS_DISTINCT", "CDAS_DEPLOYMENT_ID", "STAGING_HEALTH_PROOF_SECRET", "AI_PROVIDER_DISABLED_OR_EXPLICITLY_ENABLED", "AI_PROVIDER_SYNTHETIC_ACK", "AI_GATEWAY_CONFIG_WHEN_ENABLED",
  ],
  "staging-database.v1": [
  ...
  "staging-application.v1": [
    "APPLICATION_HEALTH_HTTP_200", "APPLICATION_HEALTH_NO_STORE", "APPLICATION_HEALTH_EXACT_BODY", "APPLICATION_DEPLOYMENT_ID_MATCHES", "APPLICATION_CONFIGURATION_PROOF_MATCHES", "APPLICATION_SOURCE_FINGERPRINT_MATCHES",
  ],
} as const;
const preflightPresentCodes = new Set([
  "STAGING_BASE_URL_HTTPS_REMOTE", "DATABASE_URL_REMOTE_POOLED", "DIRECT_URL_REMOTE_DIRECT", "DATABASE_URL_TLS_REQUIRED", "DIRECT_URL_TLS_REQUIRED", "CLERK_TEST_PUBLISHABLE_KEY", "CLERK_TEST_SECRET_KEY", "STAGING_TEST_TEACHER_CLERK_ID", "STAGING_TEST_STUDENT_CLERK_ID", "CDAS_DEPLOYMENT_ID", "STAGING_HEALTH_PROOF_SECRET", "AI_GATEWAY_CONFIG_WHEN_ENABLED",
]);
...
function isExactPassingEvidence(value: Evidence, expectedSchema: keyof typeof expectedEvidenceChecks): boolean {
  ...
  return checks.every((check) => {
    ...
    const requiresPresent = expectedSchema === "staging-preflight.v1" && preflightPresentCodes.has(item.code as string);
