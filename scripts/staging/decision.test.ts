...
const passingEvidence = (schema: keyof typeof expectedEvidenceChecks) => ({
  ...
  checks: expectedEvidenceChecks[schema].map((code) => ({
    ...
    status: "PASS",
    ...(
      schema === "staging-preflight.v1" && ["STAGING_BASE_URL_HTTPS_REMOTE", "DATABASE_URL_REMOTE_POOLED", "DIRECT_URL_REMOTE_DIRECT", "DATABASE_URL_TLS_REQUIRED", "DIRECT_URL_TLS_REQUIRED", "CLERK_TEST_PUBLISHABLE_KEY", "CLERK_TEST_SECRET_KEY", "STAGING_TEST_TEACHER_CLERK_ID", "STAGING_TEST_STUDENT_CLERK_ID", "CDAS_DEPLOYMENT_ID", "STAGING_HEALTH_PROOF_SECRET", "AI_GATEWAY_CONFIG_WHEN_ENABLED"].includes(code)
        ? { present: true }
        : {}
    ),
  })),
