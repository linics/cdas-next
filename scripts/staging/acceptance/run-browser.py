...
def base_url() -> str:
    ...
    parsed = urlparse(raw)
    if parsed.scheme != "https" or not parsed.hostname or not is_public_hostname(parsed.hostname) or parsed.path not in {"", "/"} or parsed.query or parsed.fragment or parsed.username or parsed.password:
...
def sign_in(page: Page, remote: str, role: str) -> None:
    ...
    try:
        ...
        page.wait_for_function("() => Boolean(window.Clerk?.loaded && window.Clerk.status === 'ready')", timeout=60_000)
        page.evaluate(
            """async ticket => {
