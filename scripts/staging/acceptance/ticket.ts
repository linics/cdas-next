...
export type AcceptanceTicketRole = "TEACHER" | "STUDENT" | "OTHER_STUDENT" | "OTHER_TEACHER";
...
export async function issueAcceptanceTicket(
  ...
  if (role !== "TEACHER" && role !== "STUDENT" && role !== "OTHER_STUDENT" && role !== "OTHER_TEACHER") throw new Error("STAGING_ACCEPTANCE_TICKET_ROLE_INVALID");
  if (evaluateAcceptanceReadiness(environment).status !== "PASS") throw new Error("STAGING_ACCEPTANCE_READINESS_FAILED");
  const subjectName = { TEACHER: "STAGING_TEST_TEACHER_CLERK_ID", STUDENT: "STAGING_TEST_STUDENT_CLERK_ID", OTHER_STUDENT: "STAGING_TEST_OTHER_STUDENT_CLERK_ID", OTHER_TEACHER: "STAGING_TEST_OTHER_TEACHER_CLERK_ID" } as const;
