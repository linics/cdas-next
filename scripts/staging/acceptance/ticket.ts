...
export type AcceptanceTicketRole = "TEACHER" | "STUDENT";
...
export async function issueAcceptanceTicket(
  ...
  if (role !== "TEACHER" && role !== "STUDENT") throw new Error("STAGING_ACCEPTANCE_TICKET_ROLE_INVALID");
  if (evaluateAcceptanceReadiness(environment).status !== "PASS") throw new Error("STAGING_ACCEPTANCE_READINESS_FAILED");
  const userId = environment[role === "TEACHER" ? "STAGING_TEST_TEACHER_CLERK_ID" : "STAGING_TEST_STUDENT_CLERK_ID"]?.trim();
