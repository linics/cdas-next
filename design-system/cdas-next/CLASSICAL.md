...
- 装饰性入场动画、hover 平移（含 FAB `translateY`）造成布局抖动；上述公共首页特例除外
...
- **工具结果在面板里是闭合卡（2026-08-29）**：宽页面上工具结果是通栏横带——只有上下分割线、横向零内边距，由页面自身的 `--space-8` 留白托住文字。面板没有那份留白，同一条横带会贴到面板边缘，读起来像左右被切掉的方框。面板内因此改为闭合卡：`1px solid var(--color-divider)` 四边、`--radius-md`、`--space-3` 四周内边距。**这是面板专有**，宽页面的通栏横带不动。这不是气泡墙：分区仍靠分割线，卡只给承载可点击对象的工具结果。
- **记录跟随流式输出**：面板的滚动容器在新内容到达时滚到底；教师一旦上滚离开底部（阈值 32px）即停止跟随，不把人拽回。禁止用 `scroll-behavior: smooth` —— 平滑动画的中间位置会被滚动监听误判成手动上滚，跟随随即失效。
