...
状态：启动基线（v0.1）；D-060 本地认证、会话与门禁迁移已完成源码收尾；D-061 schema v3 任务书与 D-062 教师建班/名单导入进入本地开发切片
日期：2026-09-01
...
- D-062 起，班级管理员教师可用「学号、姓名」名单为本校新学生建号：`studentNo` 在本校唯一且为 6–32 位数字，初始密码由学号派生并强制首次登录修改。既有账号只被加入班级，姓名、密码与账号状态都不因导入而改变。
...
- 由一名教师管理，且必须与该教师同校。D-062 起教师可自助在本校建班；同一教师的同名班级被拒绝。
- 教师只能删除从未存在成员区间、从未产生 Release 的空班级；任何进入过教学历史的班级必须保留。
- 是活动发布和学生可见性的边界。
...
- D-062 的 Excel 导入同样由服务端解析结果准备确认意图；未来生效的成员区间按冲突处理，不能显示成当前成员，也不能创建与其重叠的新区间。
