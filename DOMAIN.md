...
- D-044/D-055 的评价建议只允许发布教师且仍管理目标班级为当前正式修订请求；其他教师、组外学生与不可见资源继续得到资源级不存在。建议必须覆盖冻结 v2 量规全部维度，只能引用当前可见文字、本版已确认检查点或本次已经成功读取的当前修订附件 ID；不可读附件必须视为证据不足。模型调用、附件读取与解析都在数据库事务之外，非法输出和模型失败不得创建评价、确认意图或评价修订。最终准备命令只接受同教师、已成功且具有同一正式修订与评价版本成功审计的建议 AgentRun，其他成功运行不能冒充建议来源。
