...
状态：启动基线（v0.1）；D-060 本地认证、会话与门禁迁移已完成源码收尾；D-061 schema v3 任务书进入本地开发切片
日期：2026-09-01
...
- 表示经过认证的教师、学生，或唯一的平台管理员。
- 角色只决定能力范围，不代替具体资源归属检查。
- 教师与学生属于一所学校；学校赋值后不可改。ADMIN 不属于任何学校。
- 学生可由受控 operator 配置唯一、规范化的 `rosterKey`（名单码），用于教师精确导入既有账号；名单码不是认证凭证，不能代替登录、班级管理权或成员变更确认。
- D-059 起，账号与所属学校都可以停用。停用只拒绝后续业务访问，不删除教学历史。管理员可登记本校教师：写入 `AppUser` 与待开通 provisioning，不创建认证身份。

### LocalCredential（本地凭据）

- 与一个 `AppUser` 一对一绑定；规范化 identifier 全局唯一。管理员使用 `admin:<username>`，教师与学生使用带学校代码的角色命名空间，跨校相同工号或学号不会共享凭据。
- 只保存版本化 Argon2id envelope，不保存、记录或回显明文密码。密码为 10–128 个 Unicode 字符，至少包含一个字母和一个数字。
- 保存首次改密要求、最近改密时间、连续失败次数和临时锁定截止时间。连续 5 次失败锁定 15 分钟；未知 identifier 使用同参数 dummy hash，已锁账号在密码派生前拒绝。
- `TeacherProvisioning(PENDING)` 完成开通或首次改密时，既有 `AppUser` 原位从 `pending:<provisioningId>` 转为 `local:<同一 AppUser id>`；教学资源、成员关系和历史 ID 不迁移、不复制。
- LocalCredential 只证明登录凭据属于哪个 AppUser，不承载班级、学校或教学资源授权。
...
- 由一名教师管理，且必须与该教师同校。
- 是活动发布和学生可见性的边界。
- D-059 起带必填 `schoolId`；成员必须是同校学生。
- 班级的学校归属创建后不可改；跨校调整必须创建新的班级边界，不能搬动既有教学历史。

### School（学校）

- 公开代码创建后不可改；名称可改；状态为启用或停用。
- 教师邀请码只存哈希。明文只在管理员创建或重置命令的当次结果中出现。
- 教学历史表不保存 `schoolId`。
...
- D-061 起，既有 schema v1/v2 历史不补写、不迁移；新的创建必须形成完整 v3 修订。既有未封存 v2 草稿继续按 v2 编辑，保存与 Agent 改写必须保持原 schema 版本。
- v3 是独立 canonical task book：2–8 条可观察学习目标各引用 1–3 条与已选学科、学段和年级匹配的官方核心素养；`disciplineContributions` 精确覆盖主学科和融合学科；每个阶段与量规维度用 `learningGoalIds` 关联目标并共同覆盖全部目标。证据只允许 `text`、`document`、`image`、`confirm`；兼容标量列从正文派生，不由教师维护第二份内容。
- 草稿永远不对学生可见。
...
- 白名单固定为教育部《义务教育课程方案（2022年版）》及 14 门 2022 年课程标准的已圈定章节；综合实践活动不提供独立课标。每个来源和章节都有稳定 ID、标题层级、原文位置说明、来源 URL 与 SHA-256；Word 版不能可靠恢复物理页码时不得伪造页码。
...
- 当前提案中的来源引用只存在于同一 AI SDK 消息会话，教师可点开核对原文；批准草稿时仍只持久化 schema v2 `content`。因此引用不是发布快照、课程标准映射、合规结论或教育效果事实，教师编辑后也不自动重写。
...
- 发布后不可原位修改核心内容。
- snapshot schema v1、v2 与 v3 分别保持自己的精确正文和规范化 hash；旧 snapshot 不因新版本上线而回填或重算，新发布冻结完整 v3 修订。
- D-031 通过不可变 `executionVersion` 冻结执行协议：历史 Release 与新 `once` 任务使用整项协议 0；结构化 v2/v3 `phased`/`mixed` Release 使用顺序阶段协议 1。协议版本不能在发布后修改，也不能改变既有 snapshot/hash。
- 第一阶段一个发布实例只面向一个班级。
...
- D-025/D-026 增加 SubmissionAttachment：业务记录保存所属 Submission 与学生、类型、文件名、媒体类型、大小、私有 storage key、上传/验证时间和状态；文件内容保存在已配置的私有附件后端（Vercel Private Blob 或 D-055 自托管持久磁盘），storage key 不构成读取权限。
...
- 只对结构化 schema v2/v3 发布快照开放。量规来自冻结 snapshot 的 `rubricDimensions`，按 1 起始的 `dimensionIndex` 与精确 `dimensionName` 覆盖全部 4–8 个维度，不得增删或改名。不得从 v1 `feedbackCriteria` 发明维度。
...
- D-044 起，教师可在第一方评阅页请求模型为当前正式修订起草完整量规建议。D-055 起，除冻结量规、当前可见文字与已确认检查点外，当前正式修订的每个附件必须先复用第一方下载授权重新验证，成功读取后才以不含文件名和 storage key 的受限转写进入建议；不可读附件不得被引用。建议只回填可编辑表单，不是评价事实。
...
- ActivityDraft head 的内容 schema 版本和完整任务书必须与同版本不可变 revision 精确一致；更新不得改变 schema 版本。v2/v3 修订都必须满足学段—年级—学科、作业类型—子类型、阶段证据链和评价档位约束，v3 另须由应用校验与数据库冻结 registry 快照共同保证核心素养可引用性及目标—阶段—量规全覆盖。
- v1、v2 与 v3 Release snapshot 必须分别等于其精确 source revision，并按对应 schema 的固定规则计算 SHA-256；不能用新 schema 改写旧历史。
...
- 附件后端的上传、对象元数据、文件头、下载流、文本抽取与视觉模型调用必须在数据库事务外处理；永久对象 URL 与 storage key 不返回浏览器或模型，每次读取仍按学生受众或发布教师与当前班级管理关系重新授权。图片/PDF 可由授权路由 inline 预览，Word 与未知类型保持 attachment。
...
- D-035 的维度结果、综评、精确 SubmissionRevision、评价预期版本与 payload hash 必须在同一个已执行 ActionIntent、TeacherEvaluationRevision、审计及幂等结果中冻结；必须覆盖冻结 v2/v3 量规的全部维度。个人评价仅本人可见，小组评价仅该组成员共享；其他调用者得到资源级不存在。
...
- D-044/D-055 的评价建议只允许发布教师且仍管理目标班级为当前正式修订请求；其他教师、组外学生与不可见资源继续得到资源级不存在。建议必须覆盖冻结 v2/v3 量规全部维度，只能引用当前可见文字、本版已确认检查点或本次已经成功读取的当前修订附件 ID；不可读附件必须视为证据不足。模型调用、附件读取与解析都在数据库事务之外，非法输出和模型失败不得创建评价、确认意图或评价修订。最终准备命令只接受同教师、已成功且具有同一正式修订与评价版本成功审计的建议 AgentRun，其他成功运行不能冒充建议来源。
