...
Prefer the primary Sol thread as the sole writer for authorization, resource ownership, Prisma schema or migrations, database invariants, `ActionIntent`, idempotency, `AgentRun` provenance, append-only business history, and shared UI/Agent domain commands. A Luna builder may touch these areas only after the primary thread has fixed the exact invariant, file scope, negative tests, and transaction boundary.
