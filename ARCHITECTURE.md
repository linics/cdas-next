...
- `create_activity_draft` 是 D-033 的 L1 工具。资料不足时每轮只提出一个会改变设计的必要问题，且不写入。资料充分后，签名 approval 展示理解摘要、教师已提供要求、明确假设、各融合学科贡献、知识/过程/情感三条目标—任务—证据—评价链、完整 schema v2 内容，以及（命中已收录学科时）至少两条不同官方来源的可点开引用；教师批准前不执行草稿写入。批准后仍以当前教师、`AGENT` 来源和 owned RUNNING AgentRun 调用共享 `saveActivityDraft`；拒绝确认不产生草稿。该理解确认不是发布确认，也不建立 ActionIntent。成功输出包含精确 draft ID 和站内路径；客户端只在两者一致时进入预览。
