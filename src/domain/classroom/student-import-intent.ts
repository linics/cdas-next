...
export function hashStudentImportPayload(payload: StudentImportPayload): string {
  const value = canonicalize(studentImportPayloadSchema.parse(payload));
  if (value === undefined) throw new TypeError("Student import payload cannot be canonicalized");
