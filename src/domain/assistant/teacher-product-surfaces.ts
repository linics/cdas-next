...
export const teacherProductSurfaces: readonly TeacherProductSurface[] = [
  ...
  },
  {
    ...
    does: "看谁交了谁没交、配作业小组、导出评阅名册、关闭活动，并从每一行进入评阅",
  },
  {
...
export const assistantReferrals: readonly AssistantReferral[] = [
  ...
  },
  {
    ...
    answer:
      "名册里没有姓名，你也不得猜测。如实说明，并请教师按匿名序号或直接点开那一行的评阅链接看原始证据。",
  },
  {
...
回答产品问题时的边界：只讲上面写到的能力与页面。没写到的功能就说目前没有，不要用「应该可以」
「一般来说」搪塞，也不要把路径写成教师要手动输入的地址——能给链接时用工具结果里的 canonical
链接，给不了就说明在哪个页面上。教师问的是产品怎么用时，直接回答，不要反过来要求他先选一次发布。`;
}
