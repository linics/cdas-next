...
describe("student dashboard page", () => {
  ...
  it("does not touch the database when access is unavailable", async () => {
    ...
    expect(markup).toContain("使用学校代码、学号和密码继续。");
  ...
  it("renders the local student login link only when unauthenticated", async () => {
    mocks.createUiCommandContext.mockRejectedValue(
  ...
  it("keeps unprovisioned access closed without presenting external auth", async () => {
    mocks.createUiCommandContext.mockRejectedValue(
  ...
  it("groups real release links by progress without placeholder buttons", async () => {
    ...
    expect(markup).not.toContain("data-clerk-sign-out");
