    ...
    <form id="activity-draft-form" data-hydrated={hydrated ? "true" : "false"} className={styles.editorForm} action={formAction}>
      ...
      <Section number={1} title="基本设置" detail="沿用原版 CTS 的学段、学科、作业类型和周期；截止时间仍在发布时设置。">
        <div className={styles.taskGrid}>
          ...
          <label>探究深度<select value={activeValues.inquiryDepth} onChange={(event) => update("inquiryDepth", event.target.value as ActivityContentV2["inquiryDepth"])} disabled={disabled}>{inquiryDepths.map((depth) => <option value={depth.code} key={depth.code}>{depth.label}：{depth.description}</option>)}</select></label>
