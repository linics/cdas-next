        ...
        <div className={styles.previewLayout}>
          <article className={styles.snapshotSheet}>
            ...
            {content.schemaVersion === 2 ? <>
              ...
              <section><h3>基本信息</h3><p>{content.topic}</p><div className={styles.factTags}><span>{content.schoolStage === "PRIMARY" ? "小学" : "初中"} {content.grade} 年级</span><span>主学科 {disciplineLabel(content.mainDisciplineCode)}</span><span>融合 {content.integratedDisciplineCodes.map(disciplineLabel).join(" · ")}</span><span>{assignmentTypeDetails(content.assignmentType).label}</span>{content.inquiryDepth ? <span>{inquiryDepths.find((item) => item.code === content.inquiryDepth)?.label}</span> : null}<span>{submissionModes.find((item) => item.code === content.submissionMode)?.label}</span><span>{content.durationWeeks} 周</span></div></section>
              <section><h3>学习目标与跨学科设计</h3><p>{content.backgroundSetting}</p><ul>{content.disciplineContributions.map((item) => <li key={item.disciplineCode}><strong>{disciplineLabel(item.disciplineCode)}</strong>：贡献 {item.contribution}；不可替代性 {item.necessity}</li>)}</ul><ol>{content.learningGoals.map((goal) => <li key={goal.id}>{goal.description}</li>)}</ol></section>
              <section><h3>总体任务</h3><p>{content.taskInstructions}</p></section>
              <section><h3>阶段任务与证据</h3><ol>{content.phases.map((phase) => <li key={phase.name}><strong>{phase.name}</strong><br />行动：{phase.action}<br />关联目标：{phase.learningGoalIds.join("、")}<br />证据：{phase.evidence.map((evidence) => `${evidenceTypeLabel(evidence.type)}：${evidence.description}`).join("；")}</li>)}</ol></section>
              <section><h3>评价量规</h3><ul>{content.rubricDimensions.map((dimension) => <li key={dimension.name}><strong>{dimension.name}</strong>（关联 {dimension.learningGoalIds.join("、")}）：优秀 {dimension.excellent}；良好 {dimension.good}；达标 {dimension.pass}；需改进 {dimension.improve}</li>)}</ul></section>
            </> : <>
