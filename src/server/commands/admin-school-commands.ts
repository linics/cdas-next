...
export async function createSchool(
  ...
  for (let attempt = 1; attempt <= serializableRetryAttempts; attempt += 1) {
    try {
      const result = await database.$transaction(
        async (transaction) => {
          ...
          const existingIdempotency =
            await transaction.idempotencyRecord.findUnique({
              where: {
          ...
          if (existingIdempotency) {
            ...
            const replay = createSchoolReplaySchema.parse(
          ...
          const schoolCode = await findUnusedSchoolCode(
            transaction,
            randomness,
          );
      ...
      if (
        error instanceof SchoolAdminCommandError &&
...
export async function setSchoolStatus(
  ...
  const result = await database.$transaction(async (transaction) => {
    ...
    if (existing) {
      ...
      return schoolStatusReplaySchema.parse(existing.response);
    }
    ...
    if (!school) {
      throw new SchoolAdminCommandError("NOT_FOUND");
    }
    const updated =
...
export async function updateSchoolName(
  ...
  const result = await database.$transaction(async (transaction) => {
    ...
    if (existing) {
      requireMatchingRequest(existing, requestHash);
      return schoolNameReplaySchema.parse(existing.response);
    }
    const school = await transaction.school.findUnique({
      where: { id: input.schoolId },
...
export async function resetSchoolTeacherInvite(
  ...
  const result = await database.$transaction(async (transaction) => {
    ...
    if (existing) {
      requireMatchingRequest(existing, requestHash);
      const replay = schoolInviteReplaySchema.parse(existing.response);
      return {
        ...replay,
        status: "EXISTING" as const,
        teacherInviteCode: null,
    ...
    const school = await transaction.school.findUnique({
      where: { id: input.schoolId },
      select: { id: true, code: true, teacherInviteCodeHash: true },
    });
    if (!school) {
      throw new SchoolAdminCommandError("NOT_FOUND");
    }
    const updated = await transaction.school.updateMany({
      where: {
        id: school.id,
        teacherInviteCodeHash: school.teacherInviteCodeHash,
      },
