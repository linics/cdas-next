...
export function createActivityAssistantTools({
  ...
  return {
    ...
    create_activity_draft: tool({
      description:
        "把教师已经说明清楚的完整跨学科任务书储存成可预览、可继续编辑的活动草稿。content.schemaVersion 必须是数字 2。作业类型只用 practical、inquiry、project；practical 子类型只用 visit、simulation、observation；inquiry 子类型只用 literature、survey、experiment；project 的 assignmentSubtype 必须为 null。融合学科贡献必须与 content.integratedDisciplineCodes 恰好一一对应，且不得包含主学科。sourceReferences 每一条都必须是本轮 read_source_section 已返回 FOUND 的章节，数量不得超过已通读章节。必须包含三维目标、三至四个连续阶段、类型化证据及四档量规，不能臆造缺失事实。backgroundSetting 必须含学生角色、真实受众和驱动性问题；各阶段 context 用第二人称承接上一阶段，写成同一个故事的连续剧集。evidenceAndAssessment 须自检三阶段是否同一故事、证据是否逐级递进。面向教师的任务书正文使用简体中文。",
